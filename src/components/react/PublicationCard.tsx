...
export default function PublicationCard({
  ...
  const getVenue = (p: Publication) =>
    p.entryTags.booktitle || p.entryTags.journal || "";
    ...
    return parts
      .map((p) => {
        ...
        if (commaIdx !== -1) {
          ...
          else if (lowerLabel.includes("ppt") || lowerLabel.includes("slide"))
            label = "Slides";
  ...
  const getIconForLabel = (label: string, url?: string) => {
    ...
    if (l.includes("slide") || l.includes("ppt"))
      return <Presentation className="w-4 h-4" />;
    if (l.includes("poster")) return <ImageIcon className="w-4 h-4" />;
    if (l.includes("teaser") || l.includes("video"))
  ...
  const codeUrl = pub.entryTags.code || "";
  const isCodeGithub = codeUrl.toLowerCase().includes("github.com");
    ...
    <div className="p-5 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-card)] hover:shadow-md transition-all duration-200 group flex flex-col h-full">
      <div className="flex-1 min-w-0 mb-4">
        ...
        <div className="flex items-center gap-3 mt-2 flex-wrap">
          <span
            className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[var(--color-primary)] text-white"
            style={{ opacity: 0.9 }}
          >
            {pub.entryTags.year}
          </span>
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {pub.entryTags.research
              ...
              .filter(
                (t) =>
                  validTags.length === 0 ||
                  validTags.some((v) => v.toLowerCase() === t.toLowerCase()),
              )
              .map((kw) =>
                onTagClick ? (
                  <button
                  ...
                  <span
                    key={kw}
                    className="text-xs px-2 py-0.5 rounded bg-[var(--color-bg-alt)] text-[var(--color-text-muted)]"
                  >
                    {kw}
                  </span>
                ),
      ...
      <div className="mt-auto pt-3">
        <div className="flex flex-wrap gap-2">
          ...
          {parseUrls(pub.entryTags.url).map((resource, i) => (
            ...
            >
              {getIconForLabel(resource.label, resource.url)}
              <span className="capitalize">{resource.label}</span>
            </a>
          ))}
          <button
            onClick={() =>
              setExpandedBibtex(
                expandedBibtex === pub.citationKey ? null : pub.citationKey,
              )
            }
          ...
          >
            ...
            <span className="capitalize">Cite</span>
          </button>
        </div>
      </div>
